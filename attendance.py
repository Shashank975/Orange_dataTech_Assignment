...
class Attendance:
    ...
    def __init__(self,root):
        ...
        self.var_counter=StringVar()

        # This part is image labels setting start 
        # first header image  
        img=Image.open(r"D:\College Project\Face-Recognition-Attendence-System\Images_GUI\Blue6.png")
        ...
        #Counter 
        counter_label = Label(left_frame,text="Count:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        counter_label.grid(row=2,column=0,padx=5,pady=5,sticky=W)

        counter_entry = ttk.Entry(left_frame,textvariable=self.var_counter,width=15,font=("verdana",12,"bold"))
        counter_entry.grid(row=2,column=1,padx=5,pady=5,sticky=W)
        ...
        table_frame.place(x=10,y=100,width=720,height=310)
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,column=("ID","Roll_No","Name","Time","Date","Attend","Count"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport_left.heading("Count",text="Count")
        ...
        self.attendanceReport_left.column("Count",width=100)
        ...
        right_frame.place(x=765,y=10,width=740,height=483)
        ...
        table_frame.place(x=10,y=100,width=717,height=360)
        ...
        #create table 
        self.attendanceReport = ttk.Treeview(table_frame,column=("ID","Roll_No","Name","Time","Date","Attend","Count"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport.heading("Count",text="Count")
        ...
        self.attendanceReport.column("Count",width=100)
    ...
    #Update button
        ...
        del_btn.grid(row=0,column=1,padx=8,pady=12,sticky=W)
    #Update button
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status" or self.var_counter.get()=="":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update stdattandance set std_id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s,std_counter=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status" or self.var_counter.get()=="":
            ...
            try:
                conn = mysql.connector.connect(user='root', password='Jushank@2021',host='localhost',database='face_recognition',port=3306)
                mycursor = conn.cursor()
                mycursor.execute("insert into stdattandance values(%s,%s,%s,%s,%s,%s,%s)",(
                self.var_id.get(),
                self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
