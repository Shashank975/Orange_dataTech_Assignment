...
from stdregister import stdRegister
...
class stdLogin:
    def __init__(self, root):
        ...
        self.bg = ImageTk.PhotoImage(file=r"D:\College Project\Face-Recognition-Attendence-System\Images_GUI\t_bg1.jpg")
        ...
        img1 = Image.open(r"D:\College Project\Face-Recognition-Attendence-System\Images_GUI\log1.png")
        img1 = img1.resize((100, 100), Image.LANCZOS)
        self.photoimage1 = ImageTk.PhotoImage(img1)
        lb1img1 = Label(image=self.photoimage1, bg="#002B53")
        lb1img1.place(x=730, y=175, width=100, height=100)

        get_str = Label(frame1, text="Student Login", font=("times new roman", 23, "bold"), fg="white", bg="#002B53")
        ...
        pwd = lb1 = Label(frame1, text="Password:", font=("times new roman", 15, "bold"), fg="white", bg="#002B53", activeforeground="white", activebackground="#007ACC")
    ...
    def reg(self):
        ...
        self.app = stdRegister(self.new_window)
    ...
    def login(self):
        if (self.txtuser.get() == "" or self.txtpwd.get() == ""):
            ...
            mycursor.execute("select * from stdlogin where email=%s and pwd=%s", (
            self.txtuser.get(),
            self.txtpwd.get()
            ))
            ...
            if row == None:
                ...
                open_min = messagebox.askyesno("YesNo", "Access only to Student Only")
                if open_min > 0:
                    self.new_window = Toplevel(self.root)
                    self.app = Face_Recognition_System(self.new_window)
                    self.root.destroy()  # Destroy the stdLogin window
                else:
                    if not open_min:
                        return
            conn.commit()
...
#=======================Reset Passowrd Function=============================
    def reset_pass(self):
        if self.var_ssq.get()=="Select":
            ...
            conn = mysql.connector.connect(user='root', password='Jushank@2021',host='localhost',database='face_recognition',port=3306)
            mycursor = conn.cursor()
            query=("select * from login where email=%s and ss_que=%s and s_ans=%s")
            ...
            if row==None:
                ...
                query=("update login set pwd=%s where email=%s")
...
# =====================Forget window=========================================
    def forget_pwd(self):
        if self.txtuser.get()=="":
            ...
            conn = mysql.connector.connect(user='root', password='Jushank@2021',host='localhost',database='face_recognition',port=3306)
            mycursor = conn.cursor()
            query=("select * from stdlogin where email=%s")
...
class Face_Recognition_System:
    def __init__(self, root):
        self.root = root
        self.root.geometry("1548x800+0+0")
        ...
        bg_img.place(x=0, y=130, width=1540, height=800)
